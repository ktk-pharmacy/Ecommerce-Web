@extends('admin.layouts.master', ['title' => 'Customer Sales Monthly Report'])
   ...
   <div class="row">
      <div class="col-12">
         <div class="card">
                  ...
                  <table class="table table-centered table-nowrap mb-0">
                     ...
                     <tbody>
                        ...
                        {{--   <!-- <td>
                          ...
                          <td>{{ $order11->customername }}</td>
                          <td> {{ $order11->Month }} </td> 
                          <td> {{ $order11->categoryGroup }} </td> 
                          <td> {{ $order11->categoryName }} </td> 
                          <td> {{ $order11->totalQty }} </td>
                          <td> {{ $order11->totalamount }} </td>
                           <!-- <td>
                              {!! statusBadge($order11->status, 'ORDERSTATUS') !!}
                           </td> -->
                           <td>
                           {{--  <!-- <a href="{{ route('admin.orders.detail', $order11->id) }}" class="action-icon"> <i class="mdi mdi-eye"></i></a> -->
                              <!-- <a href="{{ route('admin.orders.edit', $order11->id) }}" class="action-icon"> <i class="mdi mdi-square-edit-outline"></i></a> --> --}}
                           </td>
                        </tr>
                        @endforeach
                     </tbody>
            ...
            @include('admin.layouts.shared.pagination', ['paginator' => $order, $order->appends($_GET)->links(), 'link_limit' => 10])
            </div> <!-- end card-body-->
         </div> <!-- end card-->
      </div> <!-- end col -->
   </div>
