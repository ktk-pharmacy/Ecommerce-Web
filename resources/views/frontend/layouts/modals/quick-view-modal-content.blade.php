...
@php
    if ($product->sell_limit == null) {
        $sell_limit_related_product = 'unlimit';
    } else {
        $sell_limit_related_product = $product->sell_limit - productCountInCart($product) - orderedCountPdt($product);
    }
@endphp
                    ...
                    <div class="modal-product-info">
                                ...
                                <li>
                                    <div class="cart-plus-minus-2">
                                        <input type="text" value="1" name="qtybutton"
                                            class="cart-plus-minus-box-2" id="cart-quantity-2">
                                    </div>
                                </li>
                                {{-- @if (productCountInCart($product) !== $product->sell_limit) --}}
                                <li>
                                    <a href="{{ customerAuth() ? 'javascript:void(0);' : route('frontend.login') . '?redirect=' . route('frontend.products.detail', $product->slug) }}"
                                        class="theme-btn-1 btn btn-effect-1" title="Add to Cart"
                                        onclick="{{ customerAuth() ? 'addToCart(this)' : '' }}" data-type="sub"
                        ...
                        <div class="ltn__social-media">
                            <ul>
                                ...
                                <li>
                                    <a href="{{ shareTo('twitter', url()->full(), $product->name) }}" target="_blank"
                                        title="Twitter">
                                        <i class="fab fa-twitter"></i>
                                    </a>
                                </li>
                                <li>
                                    <a href="{{ shareTo('linked_in', url()->full(), $product->name) }}" target="_blank"
                                        title="Linkedin">
...
<script>
    $(document).ready(function() {
        let related_product_limit = "{{ $product->sell_limit }}";
        let sell_limit_related_product = "{{ $sell_limit_related_product }}";
        if (sell_limit_related_product !== "unlimit") {
            sell_limit_related_product = Number("{{ $sell_limit_related_product }}");
        }
            ...
            if ($button.text() == "+") {
                if (sell_limit_related_product == "unlimit") {
                    ...
                    if (sell_limit_related_product > oldValue) {
                        ...
                        Toast.fire({
                            icon: 'error',
                            title: `This item can be order ${related_product_limit} daily`
                        })
                    }
                }
            } else {
